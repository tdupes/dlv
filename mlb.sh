#!/bin/sh
# mlb: Read the Word of God in multiple languages from your terminal
# License: Public domain

SELF="$0"
VERSE_FILES="vul.tsv kjv.tsv"
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data $VERSE_FILES | awk -v cmd=list "$(get_data filter.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data $VERSE_FILES | awk -v cmd=ref -v ref="$ref" "$(get_data filter.awk)" | ${PAGER}
	done
	exit 0
fi

get_data $VERSE_FILES 2>/dev/null | awk -v cmd=ref -v ref="$*" "$(get_data filter.awk)" 2>/dev/null | ${PAGER}
